...
def main(argv):
  ...
  foutput.write("REGION_UMA,")
  foutput.write("DIST_SITIO,")
  foutput.write("NOMBRE_SITIO,")
  foutput.write("LONG_SITIO,")
  foutput.write("LAT_SITIO")
  foutput.write("\n")
    ...
    for uma in umasrc:
      ...
      with fiona.open(stationfile_g) as stationsrc:
        for station in stationsrc:
          ...
          #~ distance_between_pts = stationgeom.distance(umageom)  // Distancia en Metros
          distance_between_pts = haversine(long_uma,lat_uma,long_station,lat_station);  
          
          if ( distance_between_pts < distancia_minima):
            station_name_min = station['properties']['Name'].encode('utf-8') 
            distancia_minima = distance_between_pts;
            long_station_min = long_station
            lat_station_min  = lat_station
      ...
      print "Punto: " + str(contadorPuntos) + " Index UMA: " + str(indexuma) + "Station: " + station_name_min + " Distancia Minima: " + str(distancia_minima) + " Distancia Maxima: " + str(distancia_maxima);
          
      foutput.write(str(indexuma)+",")
      foutput.write(str(long_uma)+",")
      foutput.write(str(lat_uma)+",")
      foutput.write(regionuma+",")
      foutput.write(str(distancia_minima)+",")
      foutput.write("\""+str(station_name_min)+"\",")
      foutput.write(str(long_station_min)+",")
      foutput.write(str(lat_station_min))
      foutput.write("\n")
